...
class MonitorWindow(tk.Toplevel):
    def __init__(self, master: 'Application', image_path, fg_color, bg_color, cutoff, close_cb):
    ...
    def get_last_created_directory(self, path):
        while not self.is_closed:
            ...
            if directories:
                ...
                if current_time - timestamp < 1:  # Check if the directory is created within the last 1 seconds
                    return latest_dir_path
                print('Waiting for recent directory...')
            time.sleep(1)

    def process_new_files(self, directory):
        if self.is_closed or not os.path.exists(directory):
            return
        ...
        for file_name in new_files:
            if file_name != "config.json":
                ...
                if os.path.isfile(file_path):
                    self.drawing_process = subprocess.Popen(
                        ["python", "single_output_image_generator.py", directory, file_path, self.fg_color,
                         self.bg_color, str(self.cutoff)])
                    self.drawing_process.wait()
    ...
    def on_destroy(self):
        self.is_closed = True
        self.drawing_process.terminate()
        self.drawing_process.wait()
